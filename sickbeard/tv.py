...
class TVShow(object):
	...
	def writeEpisodeNFOs (self):
		...
		logger.log("SELECT * FROM tv_episodes WHERE showid = " + str(self.tvdbid) + " AND location != ''")
...
class TVEpisode:
	...
	def checkForMetaFiles(self): 
		...
		if ek.ek(os.path.isfile, self.location):
			...
			if ek.ek(os.path.isfile, ek.ek(os.path.join, self.show.location, helpers.replaceExtension(self.location, 'srt'))):
				self.hassrt = True
			else:
				self.hassrt = False
	...
	def loadFromTVDB(self, season=None, episode=None, cache=True):
		...
		if not os.path.isdir(self.show._location):
			logger.log("The show dir is missing, not bothering to change the episode statuses since it'd probably be invalid - " + self.show.name)
			return
	...
	def createMetaFiles(self, force=False):
		...
		logger.log("CreateMetaFiles for " + self.show._location)
	...
	def createArt(self, epsToWrite, force=False):
		...
		if not self.hassrt or force:
			if subFilename != None and periscopeImport == 1:
				if ek.ek(os.path.isfile, self.location):
					foundSubtitle = periscope.Periscope().downloadSubtitle(self.location, None)
					if not foundSubtitle:
						logger.log("Unable to download subtitle from "+subFilename, logger.ERROR)
						return None
